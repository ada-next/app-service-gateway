...
import {checkBoxIcon, checkBoxOutlineBlankIcon, closeIcon, recordIcon} from "../icons";
...
class MixService extends Service {
    ...
    onupdate(current, info) {
        current.option = Object.assign({}, current.option, info.option);
        tool.extend(current, util.mix(info.data, current.option));
        return current;
    }

    @action("toggle")
    toggle(current, row) {
        console.log(current.option.unique);
